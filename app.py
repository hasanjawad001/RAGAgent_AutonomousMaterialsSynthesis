        ...
        if st.session_state.graph is not None:
            try:
                ...
                if graph_context_chunks:
                    ...
                    # ]                    
                    merged_graph_text = "\n".join(graph_context_chunks)
                    graph_docs = [
                        Document(
                            page_content=merged_graph_text,
                            metadata={"source": "knowledge_graph", "original_content": merged_graph_text}
                        )
                    ]                    
                    ##
                    static_graph_retriever = StaticGraphRetriever()                
                    multi_query_graph = MultiQueryRetriever.from_llm(
                        retriever=static_graph_retriever,
                        llm=llm_expander,
                        include_original=True,
                    )
                    compressed_retriever_graph = ContextualCompressionRetriever(
                        base_retriever=multi_query_graph,
                        base_compressor=compressor,
                    )
                    top_docs = compressed_retriever_graph.invoke(query)
                    top_docs = reorder.transform_documents(top_docs)                    
                    ##
